    ...
    for (let i = 0; i < testLocations.length; i++) {
        ...
        const device = await prisma.device.create({
            data: {
                ...
                status: i === 0 ? client_1.DeviceStatus.DEPLOYED : client_1.DeviceStatus.WAREHOUSE,
        ...
        const meter = await prisma.meter.create({
            data: {
                ...
                status: i === 3 ? client_1.MeterStatus.MAINTENANCE : client_1.MeterStatus.ACTIVE,
                latitude: loc.lat,
                longitude: loc.lng,
        ...
        if (i === 1) {
            await prisma.alarm.create({
                data: {
                    tenantId: tenant.id,
                    meterId: meter.id,
                    type: client_1.AlarmType.LOW_BATTERY,
...
async function test2_DashboardStats(ctx) {
    const response = await apiRequest('GET', '/api/v1/dashboard/stats', undefined, ctx.accessToken);
...
async function test3_DashboardMap(ctx) {
    const response = await apiRequest('GET', '/api/v1/dashboard/map', undefined, ctx.accessToken);
    if (response.status !== 200) {
        return {
    ...
    const hasDeviceInfo = mapData.some((m) => m.batteryLevel !== null || m.signalStrength !== null);
    ...
    return {
        ...
        data: mapData.slice(0, 2),
    };
}
async function test4_DashboardAlarms(ctx) {
    const response = await apiRequest('GET', '/api/v1/dashboard/alarms?limit=10', undefined, ctx.accessToken);
    if (response.status !== 200) {
        return {
    ...
    const hasValidStructure = alarms.length === 0 ||
...
async function test5_DashboardConsumption(ctx) {
    const response = await apiRequest('GET', '/api/v1/dashboard/consumption?days=30', undefined, ctx.accessToken);
    if (response.status !== 200) {
        return {
    ...
    const hasValidStructure = consumption.length === 0 ||
