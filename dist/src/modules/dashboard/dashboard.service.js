...
let DashboardService = DashboardService_1 = class DashboardService {
    ...
    async getStats(user, tenantId) {
        ...
        const [totalMeters, totalCustomers, totalSubscriptions, readingsStats, activeAlarms, metersInMaintenance, metersOffline, totalDevices, devicesInWarehouse, devicesDeployed,] = await Promise.all([
            ...
            this.prisma.device.count({
                where: { ...tenantFilter, status: client_1.DeviceStatus.WAREHOUSE },
            }),
            this.prisma.device.count({
                where: { ...tenantFilter, status: client_1.DeviceStatus.DEPLOYED },
            }),
        ]);
        return {
            totalMeters,
        ...
        try {
            const subscriptions = await this.prisma.subscription.findMany({
                ...
                select: {
                    ...
                    meters: {
                        where: { status: client_1.MeterStatus.ACTIVE },
                        select: {
                            id: true,
                            serialNumber: true,
                            status: true,
            ...
            for (const subscription of subscriptions) {
                for (const meter of subscription.meters) {
                    ...
                    const customerName = this.extractCustomerName(subscription.customer?.customerType, subscription.customer?.details);
                    mapData.push({
                        id: meter.id,
    ...
    async getAlarms(user, tenantId, limit = 20) {
        ...
        try {
            ...
            return alarms.map((alarm) => ({
                ...
                customerName: this.extractCustomerName(alarm.meter.subscription?.customer?.customerType, alarm.meter.subscription?.customer?.details),
            }));
        }
        catch (error) {
