/**
 * =============================================================================
 * Read Water - COMPREHENSIVE CRUD VERIFICATION SCRIPT
 * =============================================================================
 *
 * This script performs a complete CRUD audit for all 7 entities:
 *   1. Tenants (Create, Read, Update/Patch, Delete)
 *   2. Users (Create, Read, Update/Patch, Delete)
 *   3. Meter Profiles (Create, Read, Update/Patch, Delete)
 *   4. Device Profiles (Create, Read, Update/Patch, Delete)
 *   5. Customers (Create, Read, Update/Patch, Delete)
 *   6. Devices (Create, Read, Update/Patch, Delete)
 *   7. Meters (Create, Read, Update/Patch, Delete)
 *
 * Usage:
 *   cd api
 *   npx ts-node scripts/verify-all-crud.ts
 *
 * Options:
 *   --url=http://...   API base URL (default: http://localhost:4000)
 *   --cleanup          Remove test data after completion (default: true)
...
interface TestContext {
  accessToken?: string;
  rootTenantId?: string;
  rootTenantPath?: string;
  // Test entities (to be cleaned up)
  testTenantId?: string;
  testUserId?: string;
  testMeterProfileId?: string;
  testDeviceProfileId?: string;
...
async function testAuth(ctx: TestContext): Promise<boolean> {
  console.log('\n📋 AUTHENTICATION\n');
...
async function testTenantsCRUD(ctx: TestContext): Promise<void> {
  console.log('\n📋 TENANTS CRUD\n');
  ...
  const readRes = await apiRequest(
    'GET',
    `/api/v1/tenants/${ctx.testTenantId}`,
    undefined,
    ctx.accessToken,
  );
  ...
  const listRes = await apiRequest(
    'GET',
    '/api/v1/tenants?page=1&limit=10',
    undefined,
    ctx.accessToken,
  );
...
async function testUsersCRUD(ctx: TestContext): Promise<void> {
  console.log('\n📋 USERS CRUD\n');
...
// =============================================================================
// Test: Meter Profiles CRUD
// =============================================================================

async function testMeterProfilesCRUD(ctx: TestContext): Promise<void> {
  console.log('\n📋 METER PROFILES CRUD\n');
  ...
  const readRes = await apiRequest(
    'GET',
    `/api/v1/profiles/${ctx.testMeterProfileId}`,
    undefined,
    ctx.accessToken,
  );
  ...
  const listRes = await apiRequest(
    'GET',
    '/api/v1/profiles?page=1&limit=10',
    undefined,
    ctx.accessToken,
  );
...
// =============================================================================
// Test: Device Profiles CRUD
// =============================================================================

async function testDeviceProfilesCRUD(ctx: TestContext): Promise<void> {
  ...
  const readRes = await apiRequest(
    'GET',
    `/api/v1/device-profiles/${ctx.testDeviceProfileId}`,
    undefined,
    ctx.accessToken,
  );
  ...
  const listRes = await apiRequest(
    'GET',
    '/api/v1/device-profiles?page=1&limit=10',
    undefined,
    ctx.accessToken,
  );
...
async function testCustomersCRUD(ctx: TestContext): Promise<void> {
  console.log('\n📋 CUSTOMERS CRUD\n');
  ...
  const readRes = await apiRequest(
    'GET',
    `/api/v1/customers/${ctx.testCustomerId}`,
    undefined,
    ctx.accessToken,
  );
  ...
  // UPDATE (PATCH) - THE CRITICAL BUG FIX TEST
  const patchRes = await apiRequest(
    'PATCH',
    `/api/v1/customers/${ctx.testCustomerId}`,
    {
...
async function testDevicesCRUD(ctx: TestContext): Promise<void> {
  console.log('\n📋 DEVICES CRUD\n');

  // Get a device profile to use
  ...
  // Set back to WAREHOUSE for potential linking test
  await apiRequest(
    'PATCH',
    `/api/v1/devices/${ctx.testDeviceId}`,
    { status: 'WAREHOUSE' },
...
async function testMetersCRUD(ctx: TestContext): Promise<void> {
  console.log('\n📋 METERS CRUD\n');

  // Get a meter profile to use
  ...
  if (!customerId) {
    const customersRes = await apiRequest(
      'GET',
  ...
  // Test device linking using existing compatible seeded data
  // (Skip linking with test-created entities as they're not configured as compatible)
  console.log('\n📋 METER-DEVICE LINKING (using seeded compatible data)\n');

  // Get an existing meter and warehouse device that are compatible
  const existingMetersRes = await apiRequest(
    'GET',
    '/api/v1/meters?status=ACTIVE&limit=1',
    undefined,
    ctx.accessToken,
  );

  const existingDevicesRes = await apiRequest(
    'GET',
    '/api/v1/devices?status=WAREHOUSE&limit=1',
    undefined,
    ctx.accessToken,
  );

  if (
    isSuccess(existingMetersRes.status) &&
    ...
    // Only test linking if meter doesn't already have a device
    if (!existingMeter.activeDeviceId) {
      const linkRes = await apiRequest(
        'POST',
        `/api/v1/meters/${existingMeter.id}/link-device`,
        {
      ...
      // Link may fail if profiles aren't compatible - that's OK, it's business logic
      ...
      // Unlink device if we successfully linked
      if (isSuccess(linkRes.status)) {
        const unlinkRes = await apiRequest(
          'POST',
          `/api/v1/meters/${existingMeter.id}/unlink-device`,
          {
...
// =============================================================================
// Test: DELETE Operations (Run after all other tests)
// =============================================================================

async function testDeleteOperations(ctx: TestContext): Promise<void> {
  console.log('\n📋 DELETE OPERATIONS\n');

  // Delete meter first (has foreign key constraints)
  if (ctx.testMeterId) {
  ...
  // Delete tenant (last, as it may have constraints)
  if (ctx.testTenantId) {
    const deleteRes = await apiRequest(
      'DELETE',
      `/api/v1/tenants/${ctx.testTenantId}`,
...
async function runVerification(): Promise<void> {
  ...
  // 4. Meter Profiles CRUD
  await testMeterProfilesCRUD(ctx);

  // 5. Device Profiles CRUD
